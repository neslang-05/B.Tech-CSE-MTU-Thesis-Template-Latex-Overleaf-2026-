...
After merging the two datasets, we had a total of 1,563 paddy seed images divided into three quality categories. Let's look at where these images came from and how they're distributed:
...
Here's a summary of all the settings we used during training:
...
After training finished, we saved the model in multiple formats so it can run in different environments:
...
Before committing to ResNet50, we tested four popular neural network architectures on the same dataset, using the same training approach, to see which performed best. We compared ResNet50, MobileNetV2, DenseNet121, and GoogleNet. Here are the results:
...
After completing both training phases, we evaluated our final ResNet50 model on the test set—the images we'd held back and never shown the model during training. Here's how well it performed:
