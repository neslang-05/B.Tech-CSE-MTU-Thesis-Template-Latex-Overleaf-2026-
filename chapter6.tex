...
\node (farmer) [below left of=user, xshift=-1cm] {Farmer};

\node (officer) [below right of=user, xshift=1cm] {Officer};

\node (seed) [below of=farmer] {Seed};

\node (batch) [right of=seed, xshift=3cm] {Seed Batch};

\node (detection) [right of=batch, xshift=3cm] {Detection Result};

\node (complaint) [below of=batch] {Complaint};

\node (audit) [below of=detection] {Audit Log};

\draw[->, thick] (user) -- (farmer);
\draw[->, thick] (user) -- (officer);

\draw[->, thick] (seed) -- (batch);

\draw[->, thick] (batch) -- (detection);

\draw[->, thick] (farmer) -- (complaint);

\draw[->, thick] (officer) -- (complaint);

\draw[->, thick] (user) -- (audit);

\end{tikzpicture}

\caption{Simplified Entity Relationship Structure}
