...
% NOTE: Make sure to add \usepackage{enumitem} to your main.tex preamble 
% and \setlist{noitemsep} to apply this tightly spaced styling globally.
% Alternatively, [noitemsep] has been added locally below to each list environment.
...
\node[entity, right=4cm of profiles] (roles) {
    \textbf{AspNetRoles}
    \nodepart{second}
    PK: Id \\
    RoleName
...
\node[entity, right=4cm of verification] (complaints) {
...
\node[entity, right=4cm of complaints] (actions) {
...
\node[entity, right=4cm of audit] (tokens) {
    \textbf{RefreshTokens}
    \nodepart{second}
    PK: Id \\
    FK: UserId \\
...
\node[entity, below=3cm of actions] (systemlogs) {
    \textbf{SystemLogs}
    \nodepart{second}
    PK: Id \\
    Level \\
...
\draw[relation] (users) |- node[left, pos=0.75]{\scriptsize 1:N} (tokens);
\draw[relation] (verification) -- node[above]{\scriptsize 1:N} (complaints);
\draw[relation] (complaints) -- node[above]{\scriptsize 1:N} (actions);
\draw[relation] (actions) -- node[left]{\scriptsize 1:N} (audit);
...
The backend uses JWT-based authentication with role-based authorization, following the flow depicted in Figure~\fig:jwt-backend-flow}.
