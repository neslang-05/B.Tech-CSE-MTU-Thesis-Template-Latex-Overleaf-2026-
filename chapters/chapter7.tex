...
\node[fill=red!10, right=1.2cm of proxy] (backend) {\textbf{ASP.NET Backend}\\(Business \& Data)};
\node[fill=purple!10, below=of backend] (ml) {\textbf{AI Inference}\\(Model Engine)};
...
A \texttt{RoleGuard} component checks JWT claims against required permissions before mounting any protected React component, preventing privilege escalation at the route level.
...
Axios interceptors handle token expiry without user intervention. On a \texttt{401 Unauthorized} response, the interceptor pauses pending requests, runs a refresh cycle, and replays the original request with the new token. From the user's side, the session continues uninterrupted.
...
% The AgriVerify frontend connects the AI inference engine and backend services to a browser interface designed for two distinct user groups: farmers operating on mid-range Android devices with potentially unstable internet connections, and state administrators managing administrative dashboards. The selection of the technology stack---specifically Next.js~16, React~19, TypeScript~5, and Tailwind CSS---is deliberate, aiming to deliver a high-performance, maintainable application that remains responsive and intuitive even under these challenging field conditions \cite{singh2026impact}.