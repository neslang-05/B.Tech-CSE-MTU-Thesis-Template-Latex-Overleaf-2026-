...
\toprule
\textbf{Communication Path} & \textbf{What's Talking} & \textbf{How} & \textbf{What Happens \& Security} \\
\midrule
\textbf{Browser to Proxy} & User's browser $\rightarrow$ Next.js Proxy & HTTPS, with file uploads & Axios automatically adds JWT security tokens and handles token refresh if they expire \\
\addlinespace
\textbf{Proxy to Backend} & Next.js Proxy $\rightarrow$ ASP.NET API & REST over HTTPS & The proxy forwards all requests, checks data is valid, and hides the real backend addresses from browsers \\
\addlinespace
\textbf{Backend to AI} & ASP.NET API $\rightarrow$ AI Models & HTTPS or gRPC & Backend sends images, checks for blurriness/lighting issues, extracts text, and runs the ResNet50 model \\
\addlinespace
\textbf{Cloud Services} & ASP.NET API $\rightarrow$ Azure AI & Azure's official libraries & Uses Azure Custom Vision to check packaging, Azure OpenAI to write reports, and Azure Blob Storage to save images \\
\addlinespace
\textbf{Deployment} & Docker $\rightarrow$ Registry $\rightarrow$ Live & Container systems & Multi-stage Docker builds prepare the code, store it securely, and automatically deploy with scaling \\
\bottomrule
\end{tabularx}
\end{table}
